...
- (NSString*) frameworks {
  ...
  NSMutableString* bundleVersions = @"Frameworks:\n".mutableCopy;

  NSArray* frameworks = [NSBundle.allFrameworks sortedArrayUsingDescriptors:@[[NSSortDescriptor.alloc initWithKey:@"bundleIdentifier" ascending:YES]]];
  ...
  NSArray* bundles = [[NSBundle allBundles] sortedArrayUsingDescriptors:@[[[NSSortDescriptor alloc] initWithKey:@"bundleIdentifier" ascending:YES]]];
...
unsigned int getSelectorParameterCount(SEL selector) {
  ...
  const char *ch = (const char*)sel_getName(selector);
...
- (void) forwardInvocation:(NSInvocation*)anInvocation {
  ...
  __unsafe_unretained NSString* returnString;
  if (getSelectorParameterCount(selector)==0) {
    returnString = [NSString stringWithFormat:@"There is no help avavilable for '%@'",NSStringFromSelector(selector)];
    [anInvocation setReturnValue:&returnString];
  }
  else {
    returnString = @"Usage: 'help foo'";
    [anInvocation setReturnValue:&returnString];
  }
}
